...
export const IDENTITY_CLIENT_PROTOCOL_VERSION = '1' as const;
export const SESSION_STATUSES = ['signed_out', 'signing_in', 'signed_in', 'expired', 'unavailable'] as const;
export type SessionStatus = typeof SESSION_STATUSES[number];
export type ErrorCode =
  | 'AUTH_REQUIRED' | 'AUTH_FORBIDDEN' | 'AUTH_UNAVAILABLE' | 'IDENTITY_NOT_READY'
  | 'IDENTITY_INPUT_INVALID' | 'IDENTITY_ORIGIN_REJECTED' | 'IDENTITY_RATE_LIMITED'
  | 'IDENTITY_AUTH_FAILED' | 'IDENTITY_NOT_FOUND' | 'IDENTITY_DEPLOYMENT_MISMATCH'
  | 'UPSTREAM_INVALID_RESPONSE' | 'REQUEST_NOT_ALLOWED' | 'OPERATION_SUPERSEDED'
  | 'STORAGE_UNAVAILABLE' | 'SESSION_RESTARTED' | 'SIGNED_OUT' | 'DISPOSED'
...
export interface IdentityClientService {
  ...
  checkLocalAccess(kind: 'local' | 'protected'): Readonly<{ allowed: boolean; resourceAuthorization: 'not-evaluated' }>;
}
