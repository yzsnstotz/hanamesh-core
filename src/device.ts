...
export function createDevice(now = new Date()): StoredDevice {
...
export function rawPublicKey(device: StoredDevice): Buffer {
  return Buffer.from(device.publicKey, 'base64url');
}

function checkedPrivateKey(device: StoredDevice) {
  try {
    const raw = rawPublicKey(device);
    if (raw.byteLength !== 32) throw new Error('wrong public key length');
    ...
    if (!derivedRaw.equals(raw) || derivedId !== device.deviceId) throw new Error('device snapshot mismatch');
    return privateKey;
  } catch {
...
export function signWithDevice(device: StoredDevice, bytes: Uint8Array): Uint8Array {
