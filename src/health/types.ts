...
export interface CoreHealthSnapshot {
  readonly schemaVersion: 1;
  readonly profile: {readonly id: string; readonly version: string; readonly label: string; readonly digest: string};
...
export interface ObservationSource { observe(requirements: readonly SuiteComponent[]): Promise<readonly ComponentObservation[]> }
export interface HealthStore { publish(snapshot: CoreHealthSnapshot): Promise<void>; close?(): Promise<void> }
