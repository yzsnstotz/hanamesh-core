...
function HanaMeshSection(): ReactNode {
  ...
  return createElement('section', {className: 'hm-core-section', 'data-hanamesh-core': 'ready'},
    ...
    createElement(Row, {label: '账号'}, createElement('div', {className: 'hm-core-actions'}, createElement('span', null, `绑定状态：${state.session.bound === true ? '已绑定' : state.session.bound === false ? '未绑定' : '绑定后在网站查看'}`), createElement('button', {type: 'button', onClick: () => void bind()}, '去网站绑定'))),
    ...
    createElement(Row, {label: '关于'}, createElement('div', {className: 'hm-core-actions'}, createElement('span', null, 'hanamesh-core 0.2.0-rc.18 · DSH >=0.1.5-alpha.1 <0.2.0（已实测 0.1.5-alpha.1、0.1.5-rc.2）'), createElement('button', {type: 'button', onClick: () => visit('/')}, '去网站'))),
  );
}
