...
interface RegistrationResponse {deviceId: string; principalId: string; label?: DeviceLabel}
/** identity 0.2.0-rc.3 `POST /v1/identity/devices` optional `label`: hostname ≤64, os ≤32, shell ≤48 (trimmed, no control characters). */
export interface DeviceLabel {readonly hostname: string; readonly os: string; readonly shell: string}
const LABEL_LIMITS = Object.freeze({hostname: 64, os: 32, shell: 48} as const);

const clamp = (value: string, limit: number): string => value.replace(/[\u0000-\u001f\u007f]/gu, '').trim().slice(0, limit).trim();
...
export function deviceLabel(env: NodeJS.ProcessEnv = process.env): DeviceLabel | null {
  const label = {
...
async function attempt(transport: ServerTransport, device: StoredDevice, label: DeviceLabel | null): Promise<Response> {
...
export async function registerDevice(transport: ServerTransport, device: StoredDevice, label: DeviceLabel | null = deviceLabel()): Promise<RegistrationResponse> {
  ...
  if (label && response.status === 400) response = await attempt(transport, device, null);
  const result = await jsonOf<RegistrationResponse>(response);
