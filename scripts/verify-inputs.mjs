...
// Real sibling artifacts (STATUS §5 registered digests): hanamesh-usage 0.2.0-rc.6, @hanamesh/dsh-app-host 0.1.0-rc.19.
const siblingRows = (await readFile('vendor/siblings/SHA256SUMS', 'utf8')).trim().split('\n');
assert.equal(siblingRows.length, 3);
for (const row of siblingRows) {
  const match = /^([a-f0-9]{64})  (hanamesh-usage-0\.2\.0-rc\.6\.tgz|hanamesh-dsh-app-host-0\.1\.0-rc\.19\.tgz|hanamesh-lib-provision-0\.1\.0-rc\.1\.tgz)$/.exec(row);
  assert.ok(match, row);
