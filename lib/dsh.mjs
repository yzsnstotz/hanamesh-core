...
export async function apply(ctx, config) {
  ...
  const spec = defineDomain({name: 'hanamesh_core', version: 1, layout: 'single', tables: {}, global: {
    schema: z.object({schemaVersion: z.literal(1), revision: z.number().int().nonnegative(), device, registration, consent, serverObservation}).strict(),
  ...
  try {
    ...
    for (const path of Object.values(ROUTES)) {
      const methods = path === ROUTES.state || path === ROUTES.diagnostics || path === ROUTES.health ? /** @type {const} */ (['GET']) : /** @type {const} */ (['POST']);
