...
export const INITIAL_OBSERVATION = Object.freeze({ schemaVersion: 1, revision: 0, status: 'signed_out', observedAt: null, logoutPending: false });
...
export function configOf(config) {
    if (!object(config) || typeof config.identityOrigin !== 'string')
        throw new IdentityClientError('IDENTITY_INPUT_INVALID', 400);
    let url;
    try {
        url = new URL(config.identityOrigin);
    }
    catch {
        throw new IdentityClientError('IDENTITY_INPUT_INVALID', 400);
    }
    ...
    if (typeof config.deploymentId !== 'string' || !/^[A-Za-z0-9][A-Za-z0-9:._-]{0,127}$/.test(config.deploymentId))
        throw new IdentityClientError('IDENTITY_INPUT_INVALID', 400);
    if (config.requiredLogin !== undefined && typeof config.requiredLogin !== 'boolean')
        throw new IdentityClientError('IDENTITY_INPUT_INVALID', 400);
    const timeoutMs = config.timeoutMs ?? 10000;
    if (!Number.isInteger(timeoutMs) || timeoutMs < 10 || timeoutMs > 60000)
        throw new IdentityClientError('IDENTITY_INPUT_INVALID', 400);
    if (config.protectedPrefixes !== undefined && (!Array.isArray(config.protectedPrefixes) || config.protectedPrefixes.some(p => typeof p !== 'string')))
        throw new IdentityClientError('IDENTITY_INPUT_INVALID', 400);
    const prefixes = [...(config.protectedPrefixes ?? ['/v1/identity'])];
    if (prefixes.length > 32 || prefixes.some(p => !/^\/v1\/[a-z][a-z0-9_-]*(?:\/[a-z][a-z0-9_-]*)*$/.test(p)))
...
export function principalOf(value, deploymentId) {
    if (!object(value) || !object(value.principal))
...
export function requestOf(value, config) {
    ...
    if (/%(?:2e|2f|5c|25|0[0-9a-f]|7f)/i.test(path) || /(?:^|\/)\.{1,2}(?:\/|\?|$)/.test(path))
        throw new IdentityClientError('REQUEST_NOT_ALLOWED', 400);
    let url;
    try {
        decodeURIComponent(path);
        url = new URL(path, config.identityOrigin);
    }
    catch {
        throw new IdentityClientError('REQUEST_NOT_ALLOWED', 400);
    }
    ...
    if (!['GET', 'POST', 'PATCH', 'PUT', 'DELETE'].includes(method) || (method === 'GET' && value.body !== undefined))
        throw new IdentityClientError('REQUEST_NOT_ALLOWED', 400);
    let body;
    ...
    if (url.pathname.startsWith('/v1/identity')) {
        if (url.search || !((method === 'GET' && (url.pathname === '/v1/identity/me' || /^\/v1\/identity\/principals\/[0-9a-f-]{36}$/i.test(url.pathname))) ||
            (method === 'PATCH' && url.pathname === '/v1/identity/me')))
            throw new IdentityClientError('REQUEST_NOT_ALLOWED', 400);
        if (method === 'PATCH' && (!object(value.body) || Object.keys(value.body).join(',') !== 'displayName' ||
            typeof value.body.displayName !== 'string' || !value.body.displayName.trim() || value.body.displayName.length > 120))
            throw new IdentityClientError('IDENTITY_INPUT_INVALID', 400);
    }
...
export function publicJson(value, secrets = [], depth = 0) {
    if (depth > 32)
        throw new IdentityClientError('UPSTREAM_INVALID_RESPONSE');
    ...
    if (typeof value === 'string')
        return secrets.some(s => s.length > 0 && value.includes(s)) ? '[redacted]' : value;
    if (Array.isArray(value))
        return value.map(v => publicJson(v, secrets, depth + 1));
    if (object(value)) {
        const out = Object.create(null);
        for (const [key, val] of Object.entries(value))
            if (!SECRET_KEY.test(key) && !['__proto__', 'prototype', 'constructor'].includes(key))
                out[key] = publicJson(val, secrets, depth + 1);
        return out;
    }
    throw new IdentityClientError('UPSTREAM_INVALID_RESPONSE');
}
