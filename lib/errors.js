...
export function responseError(status) {
    if (status === 401)
        return new IdentityClientError('AUTH_REQUIRED', 401);
    if (status === 403)
        return new IdentityClientError('AUTH_FORBIDDEN', 403);
    if (status === 404)
        return new IdentityClientError('IDENTITY_NOT_FOUND', 404);
    if (status === 429)
        return new IdentityClientError('IDENTITY_RATE_LIMITED', 429);
    if ([400, 413, 415, 422].includes(status))
        return new IdentityClientError('IDENTITY_INPUT_INVALID', 400);
