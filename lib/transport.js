...
export class IdentityTransport {
    ...
    hasCredentials() { return this.#cookieHeader('/v1/identity/me').length > 0; }
    #cookieHeader(path) {
        const now = Date.now();
    ...
    #acceptCookies(headers) {
        ...
        for (const text of headers.getSetCookie()) {
            ...
            if (eq <= 0 || !/^[!#$%&'*+.^_`|~0-9A-Za-z-]+$/.test(name) || !/^[\x21\x23-\x2B\x2D-\x3A\x3C-\x5B\x5D-\x7E]*$/.test(value))
                throw new IdentityClientError('UPSTREAM_INVALID_RESPONSE');
            const attrs = new Map();
            for (const part of parts) {
                const [key = '', ...rest] = part.trim().split('=');
                const normalized = key.toLowerCase();
                if (attrs.has(normalized))
                    throw new IdentityClientError('UPSTREAM_INVALID_RESPONSE');
            ...
            if (!attrs.has('httponly') || !path.startsWith('/') || (domain !== undefined && domain !== origin.hostname.toLowerCase()) ||
                ...
                (name.startsWith('__Secure-') && !attrs.has('secure')))
                throw new IdentityClientError('UPSTREAM_INVALID_RESPONSE');
            ...
            if (!value || expires <= Date.now())
                ...
                next.set(key, { name, value, path, expires, secure: attrs.has('secure') });
        }
        if (next.size > 8)
            throw new IdentityClientError('UPSTREAM_INVALID_RESPONSE');
        this.#cookies = next;
    }
    async auth(kind, body, signal) {
    ...
    async resource(url, method, body, signal) {
        return this.#send(url, method, body, signal, false);
    }
    async #send(url, method, body, callerSignal, acceptCookies) {
        if (url.origin !== this.#config.identityOrigin)
        ...
        try {
            const init = { method, headers, signal, redirect: 'error', cache: 'no-store', credentials: 'omit' };
            if (body !== undefined)
                init.body = body;
            ...
            if (acceptCookies && response.status === 200) {
                if (typeof raw !== 'object' || raw === null || !('ok' in raw) || raw.ok !== true || Object.keys(raw).length !== 1)
                    throw new IdentityClientError('UPSTREAM_INVALID_RESPONSE');
