...
function HanaMeshSection() {
    ...
    return createElement('section', { className: 'hm-core-section', 'data-hanamesh-core': 'ready' }, createElement('h2', null, 'HanaMesh'), createElement('p', { className: 'hm-core-muted', 'data-hanamesh-core-hint': 'bundle-exclusive' }, '套件与单包互斥：已单独安装 hanamesh-usage 或 @hanamesh/dsh-app-host 的用户，装 hanamesh-core 前先 dsh plugin remove 它们；已装套件后再单独安装它们同样会以 duplicate loader entry id 起不来。'), error && createElement('p', { className: 'hm-core-error', role: 'alert' }, `未能保存：${error}`), createElement(Row, { label: '设备身份' }, createElement('div', { className: 'hm-core-actions' }, createElement('span', null, `设备 id：${state.deviceId.slice(0, 8)}…　注册：${registration}`), state.serverOrigin && state.registration.status !== 'registered' && createElement('button', { type: 'button', onClick: () => void post('/api/hanamesh/core/device/register') }, '重试注册'))), createElement(Row, { label: '数据授权' }, createElement('label', { className: 'hm-core-switch' }, createElement('input', { type: 'checkbox', checked: state.consent.state === 'granted', onChange: event => void post('/api/hanamesh/core/consent', { state: event.currentTarget.checked ? 'granted' : 'withheld' }) }), '允许 HanaMesh 记录并上报本设备的使用事件（安装/打开/使用/卸载；不含内容与对话）'), createElement('span', null, `当前：${state.consent.state === 'granted' ? '已开启' : '已关闭'}`), createElement('small', { className: 'hm-core-muted' }, '撤回后本地缓冲清空并向服务端发起删除；原始记录服务端保留 90 天。')), createElement(Row, { label: '账号' }, createElement('div', { className: 'hm-core-actions' }, state.session.bound === true
        ...
        : createElement('span', { 'data-hanamesh-core-bound': String(state.session.bound) }, state.session.bound === false ? '未绑定：绑定后网站才能把本设备的贡献记到你的账号' : state.serverOrigin === null ? '未连接服务端' : '绑定状态读取中…'), state.session.bound === true
    ...
    createElement('button', { type: 'button', 'data-hanamesh-core-email': 'entry', onClick: () => visit('/me') }, '邮箱绑定'))), createElement(Row, { label: '我的 Hana' }, createElement('div', { className: 'hm-core-points', 'data-hanamesh-core-points': points?.status ?? 'loading' }, points === null
        ...
        : points.status === 'unavailable'
            ...
            : points.hanas.length === 0
                ...
                : createElement(Fragment, null, createElement('span', { 'data-hanamesh-core-total': String(points.totalPoints) }, createElement('b', null, `总计 ${fen(points.totalPoints)}`), points.pendingTotal > 0 ? `　其中待绑定 ${fen(points.pendingTotal)}` : ''), ...points.hanas.map(row => createElement('span', { key: row.hanaId, 'data-hanamesh-core-hana': row.hanaId }, `${row.hanaId.slice(0, 8)}…　${fen(row.points)}${row.pending > 0 ? `（待绑定 ${fen(row.pending)}）` : ''}　`, createElement('small', { className: 'hm-core-muted' }, BREAKDOWN_LABELS.map(([key, label]) => `${label} ${Number(row.breakdown[key].toFixed(3))}`).join(' · '))))), createElement('small', { className: 'hm-core-muted', 'data-hanamesh-core-hint': 'points-not-token' }, '分不是代币：分只记录贡献，兑换比例与发币另行公布；创作者镜像按 5% 计。'), createElement('div', { className: 'hm-core-actions' }, createElement('button', { type: 'button', onClick: () => visit('/me') }, '去网站')))), createElement(Row, { label: '本设备贡献累计' }, createElement('span', null, contributions)), createElement(Row, { label: '组件' }, createElement('div', { className: 'hm-core-components' }, state.health.fault && createElement('span', { className: 'hm-core-error' }, `检查未完成（${state.health.fault}）`), ...state.components.map(row => createElement('span', { key: row.id }, `${row.label}：${componentText(row)}`)), createElement('span', { className: 'hm-core-muted', 'data-hanamesh-core-hint': 'support-dependencies' }, '支持依赖：@hanamesh/lib-provision、zod（不是插件，DSH Market 里会显示为「Installed, not active」，属正常，无需操作）'), createElement('button', { type: 'button', onClick: () => void post('/api/hanamesh/core/health/recheck') }, '重新检查'))), createElement(Row, { label: '关于' }, createElement('div', { className: 'hm-core-actions' }, createElement('span', null, 'hanamesh-core 0.2.0-rc.31 · DSH >=0.1.5-alpha.1 <0.2.0（已实测 0.1.5-alpha.1、0.1.5-rc.2）'), createElement('button', { type: 'button', onClick: () => visit('/') }, '去网站'))));
}
...
function PointsBindPrompt() {
    ...
    return createElement('dialog', { className: 'hm-core-prompt', ref: dialog, 'data-hanamesh-core-prompt': 'bind', onCancel: close }, createElement('h3', null, '绑定后这些分才归入你的账号'), createElement('p', null, `你已累计 ${fen(pending)}（待绑定）。绑定 GitHub 账号后，这些分会全部归入账号；未绑定的分有上限。分不是代币。`), createElement('div', { className: 'hm-core-actions' }, createElement('button', { type: 'button', onClick: () => { void openBindPage().catch(() => undefined); close(); } }, '去网站绑定'), createElement('button', { type: 'button', onClick: close }, '以后再说')));
}
function FooterAction({ wide }) {
    return createElement(Fragment, null, createElement('button', { type: 'button', className: 'hm-core-footer', 'data-wide': String(wide), title: 'HanaMesh', 'aria-label': '打开 HanaMesh 设置', onClick: openHanaMeshSettings }, wide ? 'HanaMesh' : 'H'), createElement(PointsBindPrompt));
}
