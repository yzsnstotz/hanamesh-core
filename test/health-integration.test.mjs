...
test('state projects health faults and duck-typed usage service readiness', async () => {
  ...
  controller.attachHealth({
    getHealth: () => ({revision: 1, mode: 'repair', fault: 'PERSISTENCE_UNAVAILABLE', components: [
      {id: 'usage', label: '使用记录', status: 'satisfied', version: '0.2.0-rc.7', requiredRange: '0.2.0-rc.7', nextStep: null},
      {id: 'app-host', label: '应用容器', status: 'incompatible', version: '0.0.1', requiredRange: '0.1.0-rc.31', nextStep: 'restore-pinned-component'},
    ]}),
  ...
  assert.equal(state.components[1].requiredRange, '0.1.0-rc.31');
});
